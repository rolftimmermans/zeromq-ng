        ...
        public:
            inline Reference(Napi::Value value)
              : persistent(Napi::Persistent(value)) {
                ...
                int32_t err = uv_async_init(uv_default_loop(), async, [](uv_async_t* async) {
    ...
    public:
        ...
        inline Outgoing(Napi::Value value) {
            if (value.IsBuffer()) {
                ...
                /* Zero-copy heuristic. There's an overhead in releasing the buffer
                   with an async call to the main thread (v8 is not threadsafe),
                   so copying small amounts of memory may be faster than releasing
                   the initial buffer asynchronously. */
                if (length > 128) {
                ...
                /* String data should first be converted to UTF-8 before we
                   ...
                   second time. */
                auto str = new std::string(value.IsString() ? value.As<Napi::String>() : value.ToString());
