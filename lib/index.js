...
module.exports = require(`../build/${process.env.ZMQ_BUILD || "Release"}/zeromq.node`)
const {Context, Socket, Observer, capability} = module.exports
...
/* TODO: This is probably more useful as a `routingId` option on `connect()`. */
Socket.defineOption(61, "String", ["connectRid", "connectRoutingId"], {read: false, on: [Router, Stream]})
...
Object.defineProperty(Observer.prototype, "emitter", {
  get: function emitter() {
    ...
    const run = async () => {
      while (!this.closed) {
        const [event, data] = await this.receive()
